...
// Google Maps API key
// Google Maps API key with localhost restrictions removed
    ...
    async function fetchLocations() {
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          console.log('First location:', JSON.stringify(data.results[0], null, 2));
          console.log('First location geometry:', data.results[0].geometry);
          console.log('First location coordinates:', data.results[0].geometry?.location);
          ...
          const validLocations = data.results.filter(location => {
            if (!location.geometry) {
              console.warn('Location missing geometry:', location);
              return false;
            }
            if (!location.geometry.location) {
              console.warn('Location missing geometry.location:', location);
              return false;
            }
            if (typeof location.geometry.location.lat !== 'number') {
              console.warn('Location has invalid lat coordinate:', location.geometry.location.lat);
              return false;
            }
            if (typeof location.geometry.location.lng !== 'number') {
              console.warn('Location has invalid lng coordinate:', location.geometry.location.lng);
              return false;
            }
            console.log('Valid location found:', location.facility_name || location.formatted_address);
            return true;
          });
          
          console.log(`${validLocations.length} valid locations with proper coordinates`);
          if (validLocations.length > 0) {
            console.log('Setting locations state with:', validLocations);
            setLocations(validLocations);
          } else {
            console.warn('No valid locations found in API response, using fallback data');
            // Use fallback data if API returned invalid locations
            setLocations(fallbackLocationData.results);
          }
        } else {
          console.warn('No locations found in data, using fallback data');
          // Use fallback data if API returned no locations
          setLocations(fallbackLocationData.results);
        }
      } catch (err) {
        ...
        setLocations(fallbackLocationData.results);
        setError(`Note: Using local data. API error: ${err.message}`);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <Layout title="Henderson Geocode Demo">
      ...
      {loading ? (
        ...
        // Only load the map when we have data
        <div>
          {window.google && (
            <GoogleMap
            ...
            >
              {DEBUG && <div className="position-absolute top-0 start-0 bg-white p-2 m-2 rounded shadow-sm" style={{zIndex: 1000}}>
